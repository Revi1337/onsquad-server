...
@Slf4j
public abstract class AbstractDiscordNotificationClient implements DiscordNotificationClient {

    protected abstract String getWebhookEndpoint();

    @Override
    public void sendNotification(Object message) {
        send(getWebhookEndpoint(), createBody(message));
    }

    @Override
    public void sendNotification(Object message, MultipartFile file) {
        send(getWebhookEndpoint(), createBody(message, file));
    }

    @Override
    public void sendNotification(Object message, List<MultipartFile> files) {
        send(getWebhookEndpoint(), createBody(message, files.toArray(MultipartFile[]::new)));
    }

    @Override
    public void sendNotification(Object message, String fileName, byte[] fileBytes) {
        send(getWebhookEndpoint(), createBody(message, fileName, fileBytes));
    }
